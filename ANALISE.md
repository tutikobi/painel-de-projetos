...
Revisão manual de `00_prompt-inicial.md`, `constitution.md`, `spec.md`, `plan.md` e `tasks.md`, feita antes e durante a implementação. O conteúdo original desses artefatos está no commit `71e8438` (`docs: artefatos de intenção`). As correções decorrentes entraram pelos Pull Requests do repositório e aparecem no histórico de cada arquivo.
...
| Toda rota exige autenticação, exceto login/registro | Sim | Teste parametrizado cobrindo todas as rotas de dados e de IA (401). **Conflito:** a renovação de JWT precisa ser acessível sem token de acesso. Constituição emendada |
...
| Chave de IA nunca no frontend | Sim | Chave lida só do ambiente no backend. O bundle não contém a chave, só o *nome* da variável (`ANTHROPIC_API_KEY`) na mensagem de ajuda. `/api/ai/status/` responde apenas `true`/`false`, e há teste garantindo que o valor não aparece na resposta |
...
| Todo endpoint que altera dados tem teste de sucesso e de erro | Sim | Testes pytest-django por endpoint (contagem atual no README) |
...
- **H2 "carrega em uma única chamada":** interpretado como "a lista de tarefas não faz uma chamada por projeto". A barra lateral de projetos é carregada à parte e recarregada quando tarefas mudam, para atualizar as contagens.
