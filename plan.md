...
- Componente `TaskBreakdownModal`: campo de texto livre pra meta → chama `/api/ai/breakdown/` → mostra lista editável (inputs de texto e data por item, botão de remover item) → botão "Adicionar ao projeto" chama `/api/ai/breakdown/confirm/`.
  - Apesar do nome, é um **painel lateral não modal**: não tem fundo bloqueante, e em telas largas a página abre espaço para ele. Durante a espera o usuário continua usando a tela (NFR de tempo de resposta); a requisição pode ser cancelada (`AbortController`).
